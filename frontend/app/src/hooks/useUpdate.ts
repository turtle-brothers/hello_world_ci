  ...
  const update = useCallback(
    async (updateProps: UpdateProps): Promise<any> => {
      ...
      if (drawing && Object.keys(drawing).length) {
        ...
        // DrawingのUpdateエンドポイント
        const endpoint = `http://localhost:3001/drawings/${drawing.id}`;
        // const endpoint = `https://dev.zumens.jp/drawings/${drawing.id}`;
      ...
      if (version) {
        ...
        // const endpoint = `https://dev.zumens.jp/drawing_versions/${version.id}`;
      ...
      if ((!drawing || !Object.keys(drawing).length) && version) {
        ...
        // const getDrawingResponse = await axios.get(`https://dev.zumens.jp/drawings/${drawingToUpdate.id}`);
        ...
        // const endpoint = `https://dev.zumens.jp/drawings/${drawingToUpdate.id}`;
